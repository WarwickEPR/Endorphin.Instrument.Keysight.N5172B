...
module IO =
    ...
    let private checkModel (RfSource instrument) = async {
    ...
    let scpiInstrument (RfSource instrument) = instrument

    /// Post a key to the instrument, then check the error queue afterwards.
    let post key (RfSource instrument) = SCPI.Checked.Set.key key instrument

    /// Set a key to a value, then check the error queue after.
    let set<'In> key (value : 'In) (RfSource instrument) = SCPI.Checked.Set.value key value instrument
    /// Write a sequency of values
    let setSeq<'In> key (values : seq<'In>) (RfSource instrument) =
    ...
    let query (parser : string -> 'Out) key (RfSource instrument) =
        SCPI.Checked.Query.Key.parsed parser key instrument
    /// Query a key and value for a value, then check the error queue.
    let queryValue<'In, 'Out> (parser : string -> 'Out) key (value : 'In) (RfSource instrument) =
    ...
    let querySeq (parser : string -> 'Out) key (RfSource instrument) =
    ...
    let connect visaTcpipAddress timeout = async {
        ...
        let rfSource = instrument :> SCPI.IScpiInstrument |> RfSource
